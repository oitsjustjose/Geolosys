...
public class DenseDeposit implements IDeposit {
    ...
    public DenseDeposit(HashMap<BlockState, Float> oreBlocks,
            HashMap<BlockState, Float> sampleBlocks, int yMin, int yMax, int size, int genWt,
                        ...
                        if (layerRadX * layerRadX + layerRadY * layerRadY < 1.0D) {
                            for (int z = zmin; z <= zmax; ++z) {
                                ...
                                if (layerRadX * layerRadX + layerRadY * layerRadY
                                    ...
                                    if (FeatureUtils.tryPlaceBlock(reader, new ChunkPos(pos),
                                            placePos, tmp, cap)) {
                                        totlPlaced++;
                                    }
                                }
    ...
    public static DenseDeposit deserialize(JsonObject json, JsonDeserializationContext ctx) {
        ...
        try {
            ...
            return new DenseDeposit(oreBlocks, sampleBlocks, yMin, yMax, size, genWt, dimFilter,
