...
public class IECompat
{
    ...
    public static void init()
    {
        ...
        for (ExcavatorHandler.MineralMix mix : list.keySet())
        {
            ...
            if (mix.name.equalsIgnoreCase("Cassiterite") && (conf.cassiterite.getChance() > 0 || conf.teallite.getChance() > 0))
            {
                ExcavatorHandler.mineralList.remove(mix);
                continue;
            }
            ...
            if (mix.name.equalsIgnoreCase("Magnetite") && (conf.hematite.getChance() > 0 || conf.limonite.getChance() > 0))
            {
                ExcavatorHandler.mineralList.remove(mix);
                continue;
            }
